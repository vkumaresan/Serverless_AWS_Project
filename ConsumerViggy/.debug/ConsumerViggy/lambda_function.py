...
# Added functions to extract named entities and output to CSV
def extract_entities(row):
    """Uses AWS Comprehend to Extract Named Entities to a DataFrame"""
...
def apply_ner(df, column="wikipedia_snippit"):
    """Uses Pandas Apply to apply Named Entity Recognition"""
...
def lambda_handler(event, context):
    ...
    # Perform NER
    df2 = apply_ner(df)
    LOG.info(f"Entities from FANG companies: {df2.to_dict()}")

    # Write result to S3
    write_s3(df=df, bucket="fangsentimentviggy", name=names)
    write_s3(df=df2, bucket="fangentitiestviggy", name=names)